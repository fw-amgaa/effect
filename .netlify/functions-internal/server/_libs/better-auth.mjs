import { e as env, B as BetterAuthError, f as filterOutputFields, g as getAuthTables, A as APIError, a as BASE_ERROR_CODES, s as safeJSONParse, i as isProduction, d as defineRequestState, c as createAuthMiddleware, n as normalizePathname, b as deprecate, h as isValidIP, j as normalizeIP, k as isTest, l as isDevelopment, m as createRateLimitKey, o as getCurrentAuthContext, p as getCurrentAdapter, w as withSpan, q as ATTR_CONTEXT, r as ATTR_HOOK_TYPE, t as queueAfterTransactionHook, u as generateId, v as runWithTransaction, x as createAuthEndpoint, S as SocialProviderListEnum, y as logger, z as hasRequestState, C as runWithRequestState, D as runWithEndpointContext, E as ATTR_OPERATION_ID, F as shouldPublishLog, G as createLogger, H as initGetModelName, I as initGetFieldName, J as socialProviders, K as getBetterAuthVersion, L as runWithAdapter, M as defineErrorCodes, N as whereOperators } from "./better-auth__core.mjs";
import { c as createKyselyAdapter, g as getKyselyDatabaseType, k as kyselyAdapter } from "./better-auth__kysely-adapter.mjs";
import { s as sql } from "./kysely.mjs";
import { c as createRandomStringGenerator, h as hex, a as createHash, b as base64Url, d as createHMAC, e as binary } from "./better-auth__utils.mjs";
import { s as srcExports } from "./@opentelemetry/semantic-conventions.mjs";
import { d as defu, c as createDefu } from "./defu.mjs";
import { h as hkdf, s as sha256, a as hexToBytes, b as scryptAsync } from "./noble__hashes.mjs";
import { h as hexToBytes$1, m as managedNonce, u as utf8ToBytes, b as bytesToHex, x as xchacha20poly1305 } from "./noble__ciphers.mjs";
import { o as optional, a as object, b as boolean, l as looseObject, c as boolean$1, s as string, d as string$1, n as number, r as record, e as any, f as array, g as email, u as union, _ as _enum, h as _undefined } from "./zod.mjs";
import { d as decodeProtectedHeader, j as jwtDecrypt, c as calculateJwkThumbprint, e as encode, a as jwtVerify, E as EncryptJWT, S as SignJWT, J as JWTExpired } from "./jose.mjs";
import { A as APIError$1, t as toResponse, k as kAPIErrorHeaderSymbol, c as createRouter$1 } from "./better-call.mjs";
import { c as createTelemetry } from "./better-auth__telemetry.mjs";
    ...
    api[key] = async (context) => {
      ...
      const run = async () => {
        ...
        return withSpan(`${methodName} ${pathName}`, {
          [srcExports.ATTR_HTTP_ROUTE]: pathName,
  ...
  for (const hook of hooks) {
    ...
    if (matched) {
      ...
      const result = await withSpan(`hook before ${path} ${hookSource}`, {
        ...
        [srcExports.ATTR_HTTP_ROUTE]: path,
  ...
  const middlewares = options.plugins?.map((plugin) => plugin.middlewares?.map((m) => {
    const middleware = (async (context) => {
      ...
      return withSpan(`middleware ${m.path} ${plugin.id}`, {
        ...
        [srcExports.ATTR_HTTP_ROUTE]: m.path,
    ...
    async onRequest(req) {
      ...
      for (const plugin of ctx.options.plugins || []) if (plugin.onRequest) {
        const response = await withSpan(`onRequest ${normalizedPath} ${plugin.id}`, {
          ...
          [ATTR_CONTEXT]: `plugin:${plugin.id}`,
          [srcExports.ATTR_HTTP_ROUTE]: normalizedPath
    ...
    async onResponse(res, req) {
      ...
      for (const plugin of ctx.options.plugins || []) if (plugin.onResponse) {
        const response = await withSpan(`onResponse ${normalizedPath} ${plugin.id}`, {
          ...
          [ATTR_CONTEXT]: `plugin:${plugin.id}`,
          [srcExports.ATTR_HTTP_ROUTE]: normalizedPath,
          [srcExports.ATTR_HTTP_RESPONSE_STATUS_CODE]: res.status
...
async function getBaseAdapter(options, handleDirectDatabase) {
  ...
  if (!options.database) {
    ...
    const { memoryAdapter } = await import("./better-auth__memory-adapter.mjs");
...
const setRoleBodySchema = object({
  userId: string().meta({ description: "The user id" }),
  role: union([string$1().meta({ description: "The role to set. `admin` or `user` by default" }), array(string$1().meta({ description: "The roles to set. `admin` or `user` by default" }))]).meta({ description: "The role to set, this can be a string or an array of strings. Eg: `admin` or `[admin, user]`" })
});
const setRole = (opts) => createAuthEndpoint("/admin/set-role", {
  ...
  if (roles) {
    ...
    for (const role2 of inputRoles) if (!roles[role2]) throw APIError.from("BAD_REQUEST", ADMIN_ERROR_CODES.YOU_ARE_NOT_ALLOWED_TO_SET_NON_EXISTENT_VALUE);
  }
...
const getUserQuerySchema = object({ id: string$1().meta({ description: "The id of the User" }) });
...
const createUserBodySchema = object({
  email: string$1().meta({ description: "The email of the user" }),
  password: string$1().optional().meta({ description: "The password of the user. If not provided, the user will be created without a credential account (useful for magic link or social login only users)." }),
  name: string$1().meta({ description: "The name of the user" }),
  role: union([string$1().meta({ description: "The role of the user" }), array(string$1().meta({ description: "The roles of user" }))]).optional().meta({ description: `A string or array of strings representing the roles to apply to the new user. Eg: "user"` }),
  data: record(string$1(), any()).optional().meta({ description: "Extra fields for the user. Including custom additional fields." })
});
...
const adminUpdateUserBodySchema = object({
  userId: string().meta({ description: "The user id" }),
...
const adminUpdateUser = (opts) => createAuthEndpoint("/admin/update-user", {
  ...
  if (Object.prototype.hasOwnProperty.call(ctx.body.data, "role")) {
    ...
    for (const role2 of inputRoles) {
      if (typeof role2 !== "string") throw APIError.from("BAD_REQUEST", ADMIN_ERROR_CODES.INVALID_ROLE_TYPE);
      if (opts.roles && !opts.roles[role2]) throw APIError.from("BAD_REQUEST", ADMIN_ERROR_CODES.YOU_ARE_NOT_ALLOWED_TO_SET_NON_EXISTENT_VALUE);
    }
...
const listUsersQuerySchema = object({
  searchValue: string$1().optional().meta({ description: 'The value to search for. Eg: "some name"' }),
  ...
  limit: string$1().meta({ description: "The number of users to return" }).or(number()).optional(),
  offset: string$1().meta({ description: "The offset to start from" }).or(number()).optional(),
  sortBy: string$1().meta({ description: "The field to sort by" }).optional(),
  sortDirection: _enum(["asc", "desc"]).meta({ description: "The direction to sort by" }).optional(),
  filterField: string$1().meta({ description: "The field to filter by" }).optional(),
  filterValue: string$1().meta({ description: "The value to filter by" }).or(number()).or(boolean$1()).or(array(string$1())).or(array(number())).optional(),
...
const listUserSessionsBodySchema = object({ userId: string().meta({ description: "The user id" }) });
...
const unbanUserBodySchema = object({ userId: string().meta({ description: "The user id" }) });
...
const banUserBodySchema = object({
  userId: string().meta({ description: "The user id" }),
  banReason: string$1().meta({ description: "The reason for the ban" }).optional(),
...
const impersonateUserBodySchema = object({ userId: string().meta({ description: "The user id" }) });
...
const revokeUserSessionBodySchema = object({ sessionToken: string$1().meta({ description: "The session token" }) });
...
const revokeUserSessionsBodySchema = object({ userId: string().meta({ description: "The user id" }) });
...
const removeUserBodySchema = object({ userId: string().meta({ description: "The user id" }) });
...
const setUserPasswordBodySchema = object({
  newPassword: string$1().nonempty("newPassword cannot be empty").meta({ description: "The new password" }),
  userId: string().nonempty("userId cannot be empty").meta({ description: "The user id" })
});
...
const userHasPermissionBodySchema = object({
  userId: string().optional().meta({ description: `The user id. Eg: "user-id"` }),
  role: string$1().optional().meta({ description: `The role to check permission for. Eg: "admin"` })
}).and(union([object({
  permission: record(string$1(), array(string$1())),
  permissions: _undefined()
}), object({
  permission: _undefined(),
  permissions: record(string$1(), array(string$1()))
})]));
