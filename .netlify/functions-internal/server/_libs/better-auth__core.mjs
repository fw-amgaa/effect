import { A as APIError$1, a as createMiddleware, b as createEndpoint } from "./better-call.mjs";
import { i as ipv4, j as ipv6, _ as _enum, d as string } from "./zod.mjs";
import { c as createRandomStringGenerator, b as base64Url, f as base64 } from "./better-auth__utils.mjs";
import { b as betterFetch } from "./better-fetch__fetch.mjs";
import { b as decodeJwt, d as decodeProtectedHeader, a as jwtVerify, i as importJWK, f as createRemoteJWKSet } from "./jose.mjs";
...
const tracer = srcExports.trace.getTracer("better-auth", "1.5.6");
...
async function createAuthorizationURL({ id, options, authorizationEndpoint: authorizationEndpoint2, state, codeVerifier, scopes, claims, redirectURI, duration, prompt, accessType, responseType, display, loginHint, hd, responseMode, additionalParams, scopeJoiner }) {
...
async function refreshAccessToken({ refreshToken, options, tokenEndpoint: tokenEndpoint2, authentication, extraParams }) {
  ...
  const { data, error } = await betterFetch(tokenEndpoint2, {
    method: "POST",
    body,
    headers
  });
...
async function validateAuthorizationCode({ code, codeVerifier, redirectURI, options, tokenEndpoint: tokenEndpoint2, authentication, deviceId, headers, additionalParams = {}, resource }) {
...
const gitlab = (options) => {
  ...
  return {
    ...
    async getUserInfo(token) {
      if (options.getUserInfo) return options.getUserInfo(token);
      const { data: profile, error } = await betterFetch(userinfoEndpoint2, { headers: { authorization: `Bearer ${token.accessToken}` } });
...
const paypal = (options) => {
  ...
  return {
    ...
    validateAuthorizationCode: async ({ code, redirectURI }) => {
      ...
      try {
        const response = await betterFetch(tokenEndpoint2, {
          method: "POST",
          headers: {
...
const initGetDefaultModelName = ({ usePlural, schema }) => {
  const getDefaultModelName = (model) => {
    if (usePlural && model.charAt(model.length - 1) === "s") {
      ...
      if (!m2) m2 = Object.entries(schema).find(([_, f]) => f.modelName === pluralessModel)?.[0];
      if (m2) return m2;
    }
  ...
  const handleFallbackJoin = async ({ baseModel, baseData, joinModel, specificJoinConfig: joinConfig }) => {
    ...
    try {
      if (joinConfig.relation === "one-to-one") result = await withSpan(`db findOne ${modelName}`, {
        [srcExports$1.ATTR_DB_OPERATION_NAME]: "findOne",
        [srcExports$1.ATTR_DB_COLLECTION_NAME]: modelName
        ...
        result = await withSpan(`db findMany ${modelName}`, {
          [srcExports$1.ATTR_DB_OPERATION_NAME]: "findMany",
          [srcExports$1.ATTR_DB_COLLECTION_NAME]: modelName
  ...
  const adapter = {
    ...
    create: async ({ data: unsafeData, model: unsafeModel, select, forceAllowId = false }) => {
      ...
      const res = await withSpan(`db create ${model}`, {
        [srcExports$1.ATTR_DB_OPERATION_NAME]: "create",
        [srcExports$1.ATTR_DB_COLLECTION_NAME]: model
    ...
    update: async ({ model: unsafeModel, where: unsafeWhere, update: unsafeData }) => {
      ...
      const res = await withSpan(`db update ${model}`, {
        [srcExports$1.ATTR_DB_OPERATION_NAME]: "update",
        [srcExports$1.ATTR_DB_COLLECTION_NAME]: model
    ...
    updateMany: async ({ model: unsafeModel, where: unsafeWhere, update: unsafeData }) => {
      ...
      const updatedCount = await withSpan(`db updateMany ${model}`, {
        [srcExports$1.ATTR_DB_OPERATION_NAME]: "updateMany",
        [srcExports$1.ATTR_DB_COLLECTION_NAME]: model
    ...
    delete: async ({ model: unsafeModel, where: unsafeWhere }) => {
      ...
      await withSpan(`db delete ${model}`, {
        [srcExports$1.ATTR_DB_OPERATION_NAME]: "delete",
        [srcExports$1.ATTR_DB_COLLECTION_NAME]: model
    ...
    deleteMany: async ({ model: unsafeModel, where: unsafeWhere }) => {
      ...
      const res = await withSpan(`db deleteMany ${model}`, {
        [srcExports$1.ATTR_DB_OPERATION_NAME]: "deleteMany",
        [srcExports$1.ATTR_DB_COLLECTION_NAME]: model
    ...
    count: async ({ model: unsafeModel, where: unsafeWhere }) => {
      ...
      const res = await withSpan(`db count ${model}`, {
        [srcExports$1.ATTR_DB_OPERATION_NAME]: "count",
        [srcExports$1.ATTR_DB_COLLECTION_NAME]: model
...
export {
  ...
  runWithRequestState as C,
  runWithEndpointContext as D,
  ATTR_OPERATION_ID as E,
  shouldPublishLog as F,
  createLogger as G,
  initGetModelName as H,
  initGetFieldName as I,
  socialProviders as J,
  getBetterAuthVersion as K,
  runWithAdapter as L,
  defineErrorCodes as M,
  whereOperators as N,
  createAdapterFactory as O,
  capitalizeFirstLetter as P,
  getBooleanEnvVar as Q,
  ENV as R,
  SocialProviderListEnum as S,
  getEnvVar as T,
  BASE_ERROR_CODES as a,
  deprecate as b,
  createAuthMiddleware as c,
  defineRequestState as d,
  env as e,
  filterOutputFields as f,
  getAuthTables as g,
  isValidIP as h,
  isProduction as i,
  normalizeIP as j,
  isTest as k,
  isDevelopment as l,
  createRateLimitKey as m,
  normalizePathname as n,
  getCurrentAuthContext as o,
  getCurrentAdapter as p,
  ATTR_CONTEXT as q,
  ATTR_HOOK_TYPE as r,
  safeJSONParse as s,
  queueAfterTransactionHook as t,
  generateId as u,
  runWithTransaction as v,
  withSpan as w,
  createAuthEndpoint as x,
  logger as y,
  hasRequestState as z
};
