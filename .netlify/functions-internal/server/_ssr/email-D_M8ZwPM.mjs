import { c as createServerRpc, d as db, a as patientTests, p as patients } from "./index-BQMbZR5s.mjs";
...
const sendTestResultEmail_createServerFn_handler = createServerRpc({
  id: "0a83f78263cee6a9108ef14950de985385ad35a5d0ea25e6cc3b585cdc5be919",
  name: "sendTestResultEmail",
  filename: "src/server/email.ts"
}, (opts) => sendTestResultEmail.__executeServer(opts));
const sendTestResultEmail = createServerFn({
  method: "POST"
}).inputValidator((data) => data).handler(sendTestResultEmail_createServerFn_handler, async ({
  data
}) => {
  const [test] = await db.select().from(patientTests).where(eq(patientTests.id, data.testId));
  if (!test) return {
    error: "Шинжилгээ олдсонгүй"
  };
  ...
  if (!patient.email) return {
    error: "Өвчтөний имэйл хаяг байхгүй байна"
  };
  try {
    const {
      error
...
<body style="margin:0;padding:0;background-color:#f4f4f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f4f5;padding:32px 16px;">
    ...
    if (error) {
      ...
      return {
        error: `Имэйл илгээхэд алдаа: ${error.message}`
      };
    }
    await db.update(patientTests).set({
      emailSent: true,
...
export {
  sendTestResultEmail_createServerFn_handler
};
